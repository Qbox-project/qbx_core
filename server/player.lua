...
function SetPlayerPrimaryJob(citizenid, jobName)
    ...
    storage.setPlayerPrimaryJob(citizenid, player.PlayerData.job)
...
function RemovePlayerFromJob(citizenid, jobName)
    ...
    if player.PlayerData.job.name == jobName then
        SetPlayerPrimaryJob(citizenid, 'unemployed')
    end
...
local function setPlayerPrimaryGang(citizenid, gangName)
    ...
    storage.setPlayerPrimaryGang(citizenid, player.PlayerData.gang)
    player.Functions.SetPlayerData('gang', player.PlayerData.gang)
...
local function removePlayerFromGang(citizenid, gangName)
    ...
    if player.PlayerData.gang.name == gangName then
        setPlayerPrimaryGang(citizenid, 'none')
    end
...
function CheckPlayerData(source, playerData)
    ...
    local playerState = source and Player(source)?.state
    local Offline = true
    if source then
...
function CreatePlayer(playerData, Offline)
    ...
    function self.Functions.SetJobDuty(onDuty)
        ...
        storage.setPlayerPrimaryJob(self.PlayerData.citizenid, self.PlayerData.job)
    ...
    function self.Functions.SetMetaData(meta, val)
        ...
        storage.setPlayerMetadata(self.PlayerData.citizenid, self.PlayerData.metadata)
        self.Functions.UpdatePlayerData()
    ...
    function self.Functions.AddMoney(moneytype, amount, reason)
        ...
        if not self.PlayerData.money[moneytype] or amount < 0 then return false end

        self.PlayerData.money[moneytype] += amount
        storage.setPlayerMoney(self.PlayerData.citizenid, self.PlayerData.money)
    ...
    function self.Functions.SetCreditCard(cardNumber)
        ...
        storage.setPlayerCharInfo(self.PlayerData.citizenid, self.PlayerData.charinfo)
        self.Functions.UpdatePlayerData()
    end
    ...
    AddEventHandler('qbx_core:server:onGangUpdate', function(gangName, gang)
        ...
        storage.setPlayerPrimaryGang(self.PlayerData.citizenid, self.PlayerData.gang)
